...
| 1.14 | `/dashboard` | Trades to close: day's trades as R marks, "Close out the day" | 08 §7, §8 | S06 | ● (restyled 2026-09-16: `.rq-row` + `.rq-track`/`.rq-fill`, real `trades.r_multiple` per row, honest empty track when the stop was never known; screenshot-verified light+dark) |
