...
## Procedure
1. **Read** `references/retrospeq-rules.md`. Find the screen's row in `retrospeq-design-system/brand/docs/inventory.md`, then pull its frame from `brand/docs/screens/<batch>.html` (e.g. `sed -n '/id="1.10"/,/class="cap"/p' …/home-onboarding.html`) — the frame is built from the real `.rq-*`/spec-named classes, so its markup transcribes directly into JSX. Don't read the whole screens file; one frame is ~30 lines.
