...
17. **Four tabs** Home · Trades · Rulebook · Performance, inside `app/(app)/AppShellNav.tsx`. **One column, widened in steps — never full-bleed** (ADR 0047, owner 2026-09-17: the app must work on any device). Phone is the design and is unconditional; `--rq-shell-max` steps 32 → 38rem (48rem) → 42rem (64rem) → 48rem (90rem), and at ≥64rem the same four tabs render as a left rail. Screens carry no width logic: put it on `.rq-shell__*` in `brand/css/`, and cap prose at `--rq-measure`. There is no desktop mockup — compose from the catalogue, never invent a desktop language.
