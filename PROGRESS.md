...
`recordOverride`'s now-higher-stakes silent-failure mode). **The
active/next task is Slice 10d part 2 — the adherence display, §5.6 UI**
(the deliberately separate follow-up this slice's own dispatch split off
from part 1). See the "Next" paragraph further down this section for the
original full reasoning on why 10d was chosen ahead of 10c (discovery).
