...
- [ ] **`InsertStrategyInput.isDefaultStrategy` has no built-in check that the target user genuinely has zero pre-existing strategies before honoring the flag — only `strategies_one_default_per_user` (a partial unique index) backstops it, and that only blocks a SECOND default, not an inappropriate FIRST one for a user who already has other, non-default strategies.** Found by retrospeq-security-reviewer, 2026-09-04, reviewing Module 03 Slice 03b (strategy CRUD + versioning). Not exploitable today — no caller anywhere in the repo currently sets `isDefaultStrategy: true` (confirmed via repo-wide grep by both the tester and security-reviewer independently), so this is a real but currently-inert gap in a parameter with no live call site yet, not an active vulnerability. **Flagged specifically for whoever wires up Module 08's silent default-strategy creation for real** (Module 08 §5.4/§8.4's "silent default strategy," blocked at Module 08's own kickoff since Module 03 didn't exist yet — see the Module 08 kickoff analysis further down this section): that future dispatch should add an explicit "does this user already have ANY strategy?" check before setting `isDefaultStrategy: true`, not rely on the uniqueness index alone.
