...
reviewer PASS 6/6, qa PASS on all 7 items).

**A real, previously-untracked Module 04 gap was found by this slice's
own QA pass, now given its own number: Slice 10e — the rule list/browsing
view (story 1.1) plus severity promote/demote/retire controls.** Slice 7
(2026-08-25) built `promoteRule`/`demoteRule`/`retireRule` as
backend-only, explicitly deferring their UI — that UI was never built,
and nothing in the ledger was tracking it as outstanding until now (grep-
confirmed: those three functions are called nowhere in `app/` except
their own definitions/tests). Full reasoning in the phase-status table's
Slice 10d part 2 entry (search "Module 04 scope gap"). **The active/next
task is Slice 10e**, chosen ahead of the still-deferred Slice 10c
(discovery, blocked on Module 05 not existing yet) because 10e is a real,
currently-buildable gap with no external blocker — a trader today has
literally no UI path to promote a rule or see their own rulebook. See the
"Next" paragraph further down this section for the original reasoning on
why 10d was chosen ahead of 10c; that same "is it blocked on something
else" logic now favors 10e over 10c too.
