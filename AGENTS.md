...
6. `retrospeq-design-system/brand/` — **the** design system. Authoritative for every visual decision (owner, 2026-09-13): amber "Instrument" tokens, `.rq-*` components, and the **76-state mockup in `brand/docs/screens/`** (backlog: `brand/docs/inventory.md`; rules: `brand/docs/guidelines.html`). `brand/docs/instrument.html` is the earlier 17-screen sketch, superseded by the screens. `modules/09-design-system.md` (indigo, IBM Plex, shadcn, Phosphor) is superseded where it disagrees.
...
Wired twice, don't fight it: `<link href="/brand/css/index.css">` in `app/layout.tsx` (`.rq-btn`, `.rq-h1`, `.rq-num`, `.rq-row`, marks, tab bar) and `app/brand-tokens/tailwind.css` (`bg-bg`, `text-ink`, `border-line`, …). `public/brand/` and `app/brand-tokens/` are **copies** of `retrospeq-design-system/brand/` — edit the source, re-sync all three. Rules that look like bugs: one `.rq-btn` per view; `.rq-btn--equal` pairs have no primary; gauges/ambient strip always visible; ratings are dots, values are steppers, nothing on a fast-capture screen takes a keyboard; `.rq-num` on every number. Every screen lives inside the app shell (`app/(app)/AppShellNav.tsx`, four tabs, phone-width column) and is built against its frame in `brand/docs/screens/<batch>.html#<inventory row>`. **UI work goes through the skills**: `/design-build` (build/restyle; its `references/retrospeq-rules.md` is the distilled authority), `/design-audit` (review), `/design-explore` (owner-invoked only). External UI/UX skills are installed at `~/Workspace/design-skills/` (provenance + security notes in its README); the repo skills vendor what they need and work without it.
